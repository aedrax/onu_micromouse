...
class QueueArray {
  ...
  private:
    ...
    static const byte initialSize = 2;
    ...
    byte size;        // the size of the queue.
    byte items;       // the number of items of the queue.

    byte head;        // the head of the queue.
    byte tail;        // the tail of the queue.
};
...
// resize the size of the queue.
template<typename T>
void QueueArray<T>::resize (const byte s) {
  // defensive issue.
  if (s <= 0)
    exit ();
